...
try:
    if vectordb:
        vectordb._persist_directory = None
    vectordb = None
    retriever = None
    qa_chain = None
...
def upload_pdf(file: UploadFile = File(...)):
    global retriever, qa_chain, vectordb
...
@app.post("/ask/")
def ask_query_post(data: QueryInput):
    global qa_chain
    if qa_chain is None:
        return {"error": "Please upload a PDF first."}
    response = qa_chain.run(data.query)
    return {"query": data.query, "answer": response}
